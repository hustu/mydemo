...
#'
#' @description
#' Generate simulated data with predefined parameter.
#'
#' @param n Sample size.
#' @param p Dimension of mediator.
#' @param r Covariance between M's error.
#' @param alpha1.8 Coefficients `alpha` of 1st-8th mediator.
#' @param beta1.8 Coefficients `beta` of 1st-8th mediator.
#'
#' @return A list contain X, M, Y, Cov.
#' @export
#'
#' @examples
#' library(mydemo)
...
#' M <- dat$M
#' COV <- dat$COV
sim_data <- function(n, p=2000, r=0.3,
  ...
  C1 <- MASS::mvrnorm(n, rep(0, 4), Sigma)              # 4 continuous confounder
